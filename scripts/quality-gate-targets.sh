...
is_scope_expansion_path() {
    ...
    actionlint:.github/actionlint.yaml | \
        ...
        zizmor:.zizmor.yml | zizmor:zizmor.yml | zizmor:.github/zizmor.yml)
        return 0
        ;;
    esac
    return 1
...
matches_kind() {
    ...
    local first_line
    case "$kind" in
    any | gitleaks | typos) return 0 ;;
    actionlint | zizmor) [[ "$path" =~ ^\.github/workflows/[^/]+\.(yaml|yml)$ ]] ;;
    ast_grep | swift) [[ "$path" == *.swift ]] ;;
    oxlint)
        [[ "${path##*/}" == eslint.config.* ]] && return 1
    ...
    osv)
        case "${path##*/}" in
        Cargo.lock | Gemfile.lock | Package.resolved | Pipfile.lock | Podfile.lock | bun.lock | bun.lockb | composer.lock | go.sum | gradle.lockfile | mix.lock | npm-shrinkwrap.json | package-lock.json | pdm.lock | pnpm-lock.yaml | poetry.lock | pubspec.lock | uv.lock | yarn.lock) return 0 ;;
        *) return 1 ;;
        esac
        ;;
    ruff) [[ "$path" == *.py ]] ;;
