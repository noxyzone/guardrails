...
oxlint_repo="$FIXTURE/oxlint-repo"
mkdir -p "$oxlint_repo"
git -C "$oxlint_repo" init -q
git -C "$oxlint_repo" config user.email fixture@example.invalid
git -C "$oxlint_repo" config user.name Fixture
printf 'export default [];\n' >"$oxlint_repo/eslint.config.js"
printf 'export const source = true;\n' >"$oxlint_repo/source.js"
git -C "$oxlint_repo" add -- eslint.config.js source.js
oxlint_output="$FIXTURE/oxlint.bin"
...
lockfile_repo="$FIXTURE/lockfile-repo"
mkdir -p "$lockfile_repo/Sources" "$lockfile_repo/.github/workflows"
git -C "$lockfile_repo" init -q
git -C "$lockfile_repo" config user.email fixture@example.invalid
git -C "$lockfile_repo" config user.name Fixture
printf 'export const ok = 1;\n' >"$lockfile_repo/app.ts"
printf 'name: CI\non: push\njobs: {}\n' >"$lockfile_repo/.github/workflows/ci.yml"
printf '{ "pins": [], "version": 2 }\n' >"$lockfile_repo/Package.resolved"
printf 'struct App {}\n' >"$lockfile_repo/Sources/App.swift"
git -C "$lockfile_repo" add -- .
lockfile_tree="$(git -C "$lockfile_repo" write-tree)"
lockfile_commit="$(printf 'lockfiles\n' | git -C "$lockfile_repo" commit-tree "$lockfile_tree")"

oxlint_output="$FIXTURE/oxlint.bin"
"$TARGETS" --repo "$lockfile_repo" --all --kind oxlint >"$oxlint_output"
assert_null_paths "$oxlint_output" "app.ts"

osv_output="$FIXTURE/osv.bin"
"$TARGETS" --repo "$lockfile_repo" --all --kind osv >"$osv_output"
assert_null_paths "$osv_output" "Package.resolved"

zizmor_output="$FIXTURE/zizmor.bin"
"$TARGETS" --repo "$lockfile_repo" --all --kind zizmor >"$zizmor_output"
assert_null_paths "$zizmor_output" ".github/workflows/ci.yml"

gitleaks_output="$FIXTURE/gitleaks.bin"
"$TARGETS" --repo "$lockfile_repo" --all --kind gitleaks >"$gitleaks_output"
assert_null_paths "$gitleaks_output" \
    ".github/workflows/ci.yml" \
    "Package.resolved" \
    "Sources/App.swift" \
    "app.ts"

printf 'rules: {}\n' >"$lockfile_repo/.oxlintrc.json"
git -C "$lockfile_repo" add -- .oxlintrc.json
oxlint_config_tree="$(git -C "$lockfile_repo" write-tree)"
oxlint_config_commit="$(
    printf 'oxlint config\n' |
        git -C "$lockfile_repo" commit-tree "$oxlint_config_tree" -p "$lockfile_commit"
)"
oxlint_config_output="$FIXTURE/oxlint-config.bin"
"$TARGETS" \
    --repo "$lockfile_repo" \
    --changed \
    --base "$lockfile_commit" \
    --head "$oxlint_config_commit" \
    --kind oxlint >"$oxlint_config_output"
assert_null_paths "$oxlint_config_output" "app.ts"

if "$TARGETS" --repo "$lockfile_repo" --all --kind eslint >/dev/null 2>"$FIXTURE/eslint-kind.err"; then
    fail "eslint kind must be removed"
fi
if "$TARGETS" --repo "$lockfile_repo" --all --kind secretlint >/dev/null 2>"$FIXTURE/secretlint-kind.err"; then
    fail "secretlint kind must be removed"
fi
