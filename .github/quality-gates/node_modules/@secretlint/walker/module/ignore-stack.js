 ...
 */
export const createRootIgnore = (cwd, extraPatterns) => {
    return {
        dir: cwd,
        matcher: ignore().add(extraPatterns),
        parent: null,
    };
};
/**
 ...
 */
const tryReadIgnoreFile = async (filePath) => {
    try {
 ...
 */
export const extendIgnore = async (parent, dir, ignoreFiles, presentNames) => {
    ...
    const contents = reads.filter((c) => c !== null && c.trim().length > 0);
 ...
 */
export const isIgnoredByChain = (chain, fullPath, isDirectory) => {
    ...
    for (let cur = chain; cur !== null; cur = cur.parent) {
...
//# sourceMappingURL=ignore-stack.js.map