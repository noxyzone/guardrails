...
import { createRootIgnore, extendIgnore, isIgnoredByChain } from "./ignore-stack.js";
import { isDynamicPattern, splitGlobRoot } from "./glob-root.js";
const toPosix = (p) => (path.sep === "\\" ? p.replaceAll("\\", "/") : p);
/**
...
const compileMatcher = (patterns) => {
    ...
    return (relPath) => {
        if (positives.length > 0 && !positives.some((m) => m(relPath)))
            return false;
        if (negatives.some((m) => m(relPath)))
            return false;
        return true;
    };
};
 ...
 */
const groupPatterns = async (cwd, patterns, noGlob) => {
    ...
    const probedLiteral = await Promise.all(patterns.map(async (raw) => {
...
const safeReaddir = async (absDir) => {
    try {
 ...
 */
const presentIgnoreNames = (entries, ignoreFiles) => {
 ...
 */
const walkSeeded = async (absDir, chain, ctx) => {
 ...
 * process entries.
 */
const walkSubdir = async (absDir, parentChain, ctx) => {
    const entries = await safeReaddir(absDir);
    if (entries === null)
        return;
 ...
 */
const resolveEntryKind = async (full, entry, followSymlinks) => {
    if (entry.isDirectory())
        return { kind: "dir", isSymlink: false };
    if (entry.isFile())
        return { kind: "file", isSymlink: false };
    ...
    if (info.isDirectory())
        return { kind: "dir", isSymlink: true };
    if (info.isFile())
        return { kind: "file", isSymlink: true };
    return null;
};
/**
 ...
 */
const processEntries = async (absDir, entries, chain, ctx) => {
    await Promise.all(entries.map(async (entry) => {
        ...
        if (isIgnoredByChain(chain, full, isDir))
            return;
        if (isDir) {
            ...
            if (ctx.matcher(relFromPatternRoot))
                ctx.results.add(toPosix(full));
        }
    }));
};
...
const handleStaticPath = async (options) => {
    ...
    if (info.isDirectory()) {
        ...
        if (isIgnoredByChain(parentChain, absPath, true))
            return;
        ...
        if (isIgnoredByChain(parentChain, absPath, false))
            return;
